	...
	public function payment()
	{
		...
		//print_r($session_data);exit;
		 if($session_data != "") {
			...
			$package_id=$session_data1['package_id']; 
...
//print_r($res);
                        if ($resultCode == "Ok") {
							...
							$data['amount_paid'] = 'Y';   // print_r($data);                       
                            $membership_package1 = $this->register_model->updateMembershipDetails($data,$user_id);
														
							$zone_id = $this->register_model->save_zone($hotel_id);
							...
							$message = str_replace('#FULL_NAME#',$user_info['first_name'].''.$user_info['last_name'], $message);
							$basePath = base_url();
