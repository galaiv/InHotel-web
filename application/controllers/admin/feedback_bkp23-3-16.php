        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($feedback_id != "" && $feedback_id != "0") {
                $arr = array(
                    ...
                    'price' => $_POST['feedback_price'],                 
                    'description' => $_POST['description'],
                     'hotel_id' => $_POST['hotel_id'],
                );
                $feedback_id = $this->feedback_model->update_feedback($arr, $feedback_id);
 $this->feedback_model->unlinkImage($feedback_id);
               $message = $this->upload_image($feedback_id);
                $this->session->set_flashdata('success', 'Feedback details updated successfully');
                ...
                $feedback_id = $this->feedback_model->update_feedback($arr);
                 
              $message =  $this->upload_image($feedback_id);
                $this->session->set_flashdata('success', 'Feedback details added successfully');
              
            }
             if (!empty($message)) {
                $this->session->set_flashdata('error', $message['error']);
                redirect('admin/feedback/detail/' . $feedback_id);
            } else {
