...
//          $_POST['status'] = "dsfsdfds";
     ...
     //   $_POST['activation_code'] = 1380479981;
        ...
        if ($res == 'N' && $_POST['user_id'] == "") {
			...
			$email12	  = $this->client_model->get_emailByid(11);
			$message12 = $email12['email_template'];
			$message12 = str_replace('#USER_NAME#',$_POST["first_name"] . ' ' . $_POST["last_name"], $message12);
			//print_r($message);					
...
//                                echo $this->config->item('absolute_url');
    ...
    function block_guest($arr) {
		...
		function terms()
	{
		$this->load->view('terms_client');
	}
    function privacy()
	{
		
		$this->load->view('privacy_client');
	}
