 ...
 //echo "hi";exit;
    ...
    function shareExperiance($arr) { //print_r($arr);exit;
		...
		for($i=0;$i<count($stafflist);$i++)
		{
				...
				$message1 = str_replace('#FEEDBACK#', $arr['experiance'], $message1);
				//print_r($message);exit;
...
//                                echo $this->config->item('absolute_url');
    ...
    function updateDrinksStatus($arr) {
		...
		if($arr['status_flag'] =='A')
			$message = "Your drink offer has been accepted by ".$from_details['first_name']." ".$from_details['last_name'];
		else if($arr['status_flag'] =='D')
			$message = "Your drink offer has been rejected by ".$from_details['first_name']." ".$from_details['last_name'];			
		$badge_count	=	$this->client_model->update_iphone_notification($_POST['to_user']);
		$this->client_model->sendPushNotification($message,$device_tocken,$badge_count);
        $result = array('status' => 'true');
        echo json_encode($result);
    }
