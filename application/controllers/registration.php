	...
	{			$this->load->model('register_model');
	 ...
	 if ($_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            $datas = array(
             ...
             $id = $this->register_model->userRegistration($datas);	
			 if($_POST['zipcode'] !="") {
			   $complete_add = $_POST['city'].",".$_POST['zipcode'];
			   $time_zone =  $this->gettimezone($complete_add);//echo $time_zone;exit;
			 if($time_zone == '')
			...
			$email12	  = $this->register_model->get_emailByid(10);
			$message12 = $email12['email_template'];
			$message12 = str_replace('#USER_NAME#',$this->input->post("first_name").' '.$this->input->post("last_name"), $message12);
			$message12 = str_replace('#HOTEL_NAME#', $this->input->post("title"), $message12);			
			//print_r($message);					
			$this->email->from($email_from);
			$this->email->to($this->config->item('admin_email'));
			$this->email->subject($email12['email_subject']);
