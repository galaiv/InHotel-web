...
class Client extends CI_Controller {
	...
	//$json = '{"function":"get_user_details","parameters":{"user_id": "6","activation_code": "","other_id" :"5"},"token":""}';
...
//          $_POST['status'] = "dsfsdfds";
    ...
    public function checkActivationCodeExpires($arr) {
        ...
        if ($res) {
            ...
            $result['activation_status'] = (empty($activation_status)) ? 'N' : 'Y';
            $result['message'] = (empty($activation_status)) ? 'Activation code expired' : 'Your code activated successfully';
            echo json_encode($result);
            exit;
        } else {
 ...
 //echo "hi";exit;
    ...
    function MessageDetails($arr) {  //print_r($arr);
        ...
        $result = array('status' => 'true', 'details' => $details,'to_room_status'=>$to_room_status,'from_room_status'=>$from_room_status);
