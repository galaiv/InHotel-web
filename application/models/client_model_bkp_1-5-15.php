 ...
 public function checkActivationCodeExpires($activation_code,$user_id) {
  ...
  /*  public function checkActivationCodeExpires($activation_code) {
    ...
    public function GetAllGuestUsers($arr) {
        ...
        $this->db->select('imm.user_id,imm.first_name,imm.status,imm.image')
                ->from('activation_code_master acm')
                ->join('in_member_master imm', 'acm.user_id=imm.user_id')
                //->where(array('acm.hotel_id' => $hotel_id, 'imm.member_type' => 1))
    ...
    public function get_user_details($user_id) {
        $this->db->select('mm.*,am.access_member_id,am.access_member_id,acm.access_start_time as access_start_date_time,acm.access_close_time as access_end_date_time,am.room');
        //$this->db->select('mm.*');
	...
	function user_vacancy_list($activation_code)
	{
			  $this->db->select('acm.access_id')
                     ->from('activation_code_master acm')
	...
	function getMessageDetails($from_id,$to_id,$pageNo)
	{
	 ...
	 $sql="SELECT chat.id
           ...
           ORDER BY chat.sent DESC LIMIT $start_limit,10";
		//echo $sql;exit;
		$query	=	$this->db->query($sql);	
	...
	function updateDrinksStatus($id,$status_flag)
	{
		...
		$this->db->select('df.*,dm.price,am.room as from_room,am1.room as room_number');
        $this->db->where('df.id',$id)
                ->from('drink_offerings df');
	    $this->db->join('drinks_master dm', 'dm.id=df.drinks_id', 'left');	
		$this->db->join('access_master am', 'am.access_member_id = df.user_id', 'left');
		$this->db->join('access_master am1', 'am1.access_member_id = df.to_user_id', 'left');
