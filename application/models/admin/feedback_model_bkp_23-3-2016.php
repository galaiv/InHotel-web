...
class feedback_model extends CI_Model {
    ...
    public function get_feedback_details($id) {
        $this->db->select('fm.*,hp.title as hotel_title')
    ...
    function update_feedback($arr, $id = '') {
        if ($id != "") {
            $this->db->where('id', $id);
            $this->db->update('feedback_master', $arr);
            return $id;
        } else {
            $this->db->insert('feedback_master', $arr);
    ...
    public function count_feedback() {
        ...
        if ($key) {
            $this->db->where("(fm.comments like '%" . $key . "%' OR inm.first_name like '%" . $key . "%' OR inm.last_name like '%" . $key . "%' OR hp.title like '%" . $key . "%')");
        }
        if ($hotel_id) {
