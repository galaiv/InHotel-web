...
//   echo $this->db->last_query();exit;
    ...
    public function checkActivationCodeExpires($activation_code, $user_id) {
       ...
       $QRY         = "SELECT   hp.time_zone
							FROM activation_code_master  acm 
							LEFT JOIN hotel_profile hp ON hp.id=acm.hotel_id 
							WHERE acm.activation_code ='$activation_code'";
					   
       $result		=	 $this->db->query($QRY);		
	   $results	    =	 $result->row_array(); 
	   date_default_timezone_set($results['time_zone']);
    ...
    public function checkActivationCodeExists($activation_code, $user_id) {
        ...
        $this->db->select('hotel_id')
                ->from('activation_code_master acm')
                ->where('activation_code="' . $activation_code . '" and acm.active="Y" and acm.user_id=' . $user_id, false, false);
        $qry = $this->db->get();
        $res = $qry->row_array();
        if (empty($res)) {
			$this->db->select('hotel_id')
					->from('activation_code_master acm')
					->where('activation_code="' . $activation_code . '" and acm.active="Y"', false, false);
			$qry1 = $this->db->get();
			$res1 = $qry1->row_array();
			if(empty($res1)) {
				return false;
...
ORDER BY r.expiry_date DESC LIMIT 1";
    ...
    public function GetAllGuestUsers($arr) {
        ...
        $this->db->select('acm.hotel_id,acm.general')
                ->from('activation_code_master acm')
        ...
        if($is_general == 'Y') {
            $this->db->select("imm.user_id,imm.first_name,imm.status,imm.image,imm.activation_id,imm.quickblox_id,'' AS room", false);
            $this->db->from("in_member_master imm");
            $this->db->where("imm.hotel_id",$hotel_id);
            $this->db->where("imm.deleted","N");
            $this->db->where("imm.member_type",1);
            $this->db->where("imm.chat_status","Y");
            $this->db->where("imm.user_id !=",$arr['user_id']);
            if(!empty($res_arr1['blocked_profile_ids'])) {
                $this->db->where(' imm.user_id not in (' . $res_arr1['blocked_profile_ids'] . ')', false, false);
            }
            if(!empty($res_arr2['user_ids'])) {
                $this->db->where(' imm.user_id not in (' . $res_arr2['user_ids'] . ')', false, false);
            }
            /*$this->db->group_by('imm.user_id');*/
            ...
            $this->db->select('imm.user_id,imm.first_name,imm.status,imm.image,am.room,imm.activation_id,imm.quickblox_id')
                    ...
                    ->where('imm.deleted','N')
                    ->where('(acm.hotel_id = ' . $hotel_id . ' AND imm.hotel_id = ' . $hotel_id . ' AND imm.member_type = 1 AND imm.chat_status = "Y" AND imm.user_id <> ' . $arr['user_id'] . ' )');
            if (!empty($res_arr1['blocked_profile_ids'])) {
	...
	public function getAllChatUsers($arr)
	{
        ...
        if($is_general == 'Y') {
            ...
            $this->db->where("imm.chat_status","Y");
            $this->db->where("imm.user_id !=",$arr['user_id']);
            $this->db->where("imm.quickblox_id !=",'');
            if(!empty($res_arr1['blocked_profile_ids'])) {
                $this->db->where(' imm.user_id not in (' . $res_arr1['blocked_profile_ids'] . ')', false, false);
            }
            if(!empty($res_arr2['user_ids'])) {
            ...
            $this->db->select('imm.user_id,imm.first_name,imm.status,imm.image,am.room,imm.activation_id,imm.quickblox_id,imm.member_type')
                    ->from('activation_code_master acm')
                    ->join('in_member_master imm', 'acm.user_id=imm.user_id OR acm.user_id = 0')
                    ->join('access_master am', 'am.id=acm.access_id')
                    ->where('(acm.hotel_id = ' . $hotel_id . ' AND imm.hotel_id = ' . $hotel_id . ' AND imm.member_type = 1  AND imm.chat_status = "Y" AND  imm.quickblox_id <> ""  AND imm.user_id <> ' . $arr['user_id'] . ')');
            if (!empty($res_arr1['blocked_profile_ids'])) {
                $this->db->where(' acm.user_id not in (' . $res_arr1['blocked_profile_ids'] . ')', false, false);
            }
            if (!empty($res_arr2['user_ids'])) {
	...
	{
		$this->db->select('acm.hotel_id,general')
                ->from('activation_code_master acm')
                ->where('acm.activation_code', $arr['activation_code']);
 ...
 // echo $this->db->last_query();	exit;
    ...
    public function get_user_details($user_id, $activation_code = '') {
        ...
        $this->db->select('mm.*,am.access_member_id,am.access_member_id,acm.access_start_time as access_start_date_time,
            acm.access_close_time as access_end_date_time,am.room');
        $this->db->from('in_member_master mm')
                ->join('activation_code_master acm', 'acm.user_id=mm.user_id OR acm.user_id=0', 'left')
                ->join('access_master am', 'am.id = acm.access_id', 'left')
                ->join('access_members ams', 'ams.id=am.access_member_id', 'left');
        $this->db->where('mm.user_id', $user_id);
