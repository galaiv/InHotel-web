jQuery.fn.calendarPicker = function(options) {
  ...
  if (typeof(options.showMonthArrows) == "undefined")
    options.showMonthArrows=true;
	
 if (typeof(options.showYearArrows) == "undefined")
    options.showYearArrows=true;
  ...
  calendar.changeDate = function(date) {
    ...
    var fillYears = function(date) {
	  ...
	  var w = parseInt((theDiv.width()-4-(options.showYearArrows?9:0)-(nc)*4)/(nc-(options.showYearArrows?2:0)))+"px";
      for (var i = year - options.years; i <= year + options.years; i++) {
       ...
       /* var span = $("<span>").addClass("calElement").attr("millis", d.getTime()).html(i).css("width",w);
        ...
        if (i == year - options.years && options.showYearArrows) { 
          span.addClass("prev");
        } else if (i == year + options.years&& options.showYearArrows) {
          span.addClass("next");
        } else { //alert("dsfsf");
    ...
    var fillMonths = function(date) {
      ...
      var w = parseInt((theDiv.width()-4-(options.showMonthArrows?12:0)-(nc)*4)/(nc-(options.showMonthArrows?2:0)))+"px";
      for (var i = -options.months; i <= options.months; i++) {
       ...
       /* var span = $("<span>").addClass("calElement").attr("millis", d.getTime()).html(options.monthNames[d.getMonth()]).css("width",w);
		...
		var span = $("<span>").addClass("calElement").attr("millis", d.getTime())
        if (i == -options.months && options.showMonthArrows) {
          span.addClass("prev");
        } else if (i == options.months && options.showMonthArrows) {
          span.addClass("next");
        } else {
